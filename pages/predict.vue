...
<script setup >
    ...
    const {data:metaData,pending:metaDataPending} = await useLazyFetch('/api/possibilities',{query:{...userInfo.value}})

    const {data,pending} = await useLazyFetch('/api/predict',{query:{...userInfo.value}})
